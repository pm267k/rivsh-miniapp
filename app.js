...
/* ---------- Сегодня ---------- */
/* Окно первого экрана — 4 недели: примерно длина сессии, но постоянное. Граница
   «до конца текущей сессии» схлопывалась бы в ноль на каникулах. */
var ОКНО_ДЕЛ=28, ПОТОЛОК_ДЕЛ=3;
function экранСегодня(){
  ...
  if(дела.length){
    var близкие=дела.filter(function(з){return з.n<=ОКНО_ДЕЛ;});
    if(близкие.length){
...
function картаПары(п,акцент,вести){
  ...
  if(вести){
    ...
    c.onclick=function(){ вибро(); открытыйПредмет=ключДис(п.дис); откудаПредмет='пары'; перейти('предметы'); };
  }
  return c;
}
...
function экранПредмета(ключ){
  ...
  var изПар=(откудаПредмет==='пары');
  var н=эл('button','назад', изПар?'← Расписание':'← Все предметы');
  н.onclick=function(){
    открытыйПредмет=null; вибро();
    if(изПар){ откудаПредмет='предметы'; перейти('пары'); return; }
    отрисовать();
  };
  эк.appendChild(н);
