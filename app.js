...
function экранСегодня(){
  ...
  if(дела.length){
    ...
    if(близкие.length){
      ...
      близкие.slice(0,ПОТОЛОК_ДЕЛ).forEach(function(з){эк.appendChild(картаЗадания(з,false,true));});
    }else{
...
/* Одна карточка задания на всех экранах (решение 11.09): название и дата в строку,
   детали по тапу. Предмет строкой сверху — там, где нет группы по предмету («Сегодня»). */
function картаЗадания(з, прошлое, сПредметом){
