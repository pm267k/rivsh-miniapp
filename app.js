...
function экранКниг(){
  ...
  /* Enter на телефоне ничего не делал: клавиатура оставалась открытой и закрывала
     список — казалось, что поиск не отвечает (поймано 10.09). Теперь Enter прячет
...
function списокКниг(){
  ...
  /* без отбора: сверху ★ к экзамену, дальше предметы с разделами внутри */
  if(!фРаздел&&!фДис){
    var важные=наш.filter(function(к){return к.з;});
    if(важные.length){
      h.appendChild(эл('div','секц','★ К экзамену · '+важные.length));
      поПредметам(h, важные, false, 'подсекц');
    }
  }
