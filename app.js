...
/* Ссылки внутри «Комментария» (решение владельца 12.09: вторая ссылка живёт в тексте,
   а не отдельной колонкой). Понимает две записи:
     [подпись](адрес)  — подпись становится ссылкой, в таблице читается как текст
     голый https://…   — линкуется целиком, как это делает и сама Google Таблица
   Текст сначала экранируется, ссылки собираются уже по экранированному — иначе
   разметка из ячейки попала бы в HTML. */
function текстСоСсылками(текст){
  var s=экр(текст);
  s=s.replace(/\[([^\]]+)\]\((https?:\/\/[^\s)]+)\)/g, function(_,п,у){
    return '<a href="'+у+'" target="_blank" rel="noopener">'+п+'</a>'; });
  s=s.replace(/(^|[\s(])(https?:\/\/[^\s<]+)/g, function(_,до,у){
