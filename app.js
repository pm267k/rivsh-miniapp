...
/* Кнопка задания — одна. Колонка «Вторая ссылка» отменена 12.09: второй переход
   живёт ссылкой в тексте «Комментария», где его и объясняют словами. */
function кнопкиЗадания(з){
  if(!з.мат) return '';
  return '<div class="низ">'+
    '<a class="кнопка главная" data-сс="'+экр(з.мат)+'">'+(своя(з.мат)?'Материалы предмета':'Открыть материал')+'</a></div>';
}
