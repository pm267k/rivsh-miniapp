...
function экранПредмета(ключ){
  ...
  // Сверху — три ближайших актуальных; прошедшие раньше стояли первыми, потому что сортировка
  // по сроку ставила минус вперёд (11.09). Ниже — ряд как у пар: всего и сколько прошло.
  var дела=у.дела.map(подготовитьДело);
  var актив=дела.filter(function(з){return з.n>=0;}).sort(function(a,b){return a.n-b.n;});
  var прошлых=дела.length-актив.length;
  if(!актив.length) эк.appendChild(эл('div','ряд нечего','Актуальных заданий нет<span class="право"></span>'));
  актив.slice(0,3).forEach(function(з){ эк.appendChild(картаЗадания(з,false,false)); });
  if(дела.length>Math.min(3,актив.length)) эк.appendChild(рядСо('Все задания предмета', дела.length+(прошлых?' · прошло '+прошлых:''),
    function(){ фСрок=''; фПредметЗ=дела[0].дис; открытыйПредмет=null; перейти('дела'); }));
