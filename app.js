...
function картаДела(з){
  // Обычный блок, не кнопка: описание показываем целиком (оно укладывается в 2–3 строки),
  // сворачивание убрано — в WebView Telegram анимация высоты вместе с эффектом нажатия
  // давала вспышку на кадр. Кликается только кнопка материала.
  var c=эл('div','карта');
  c.appendChild(эл('div','дело',
    ...
    (з.ком?'<div class="мелко">'+экр(чистыйКоммент(з.ком))+'</div>':'')+
    (з.мат?'<div class="низ"><a class="кнопка главная" data-сс="'+экр(з.мат)+'">Открыть материал</a></div>':'')));
