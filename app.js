...
/** «04.09.2026» · «11.09 · 18.09» · «17 сентября» · «январь–февраль 2027» → {дата, точная}
    Эмодзи в сроке не ставим (решение 12.09); replace ниже оставлен страховкой на старые значения. */
...
// «Организационное» — не дисциплина: у него нет пар, преподавателя и материалов,
// и в списке предметов оно смотрелось бы как пустой предмет (правка 14.09).
// Задания из него живут на «Сегодня» и в «Заданиях» отдельным блоком.
var СЛУЖЕБНЫЕ={'общее для':1,'дисциплины, которые':1,'организационное':1,'':1};
var ОРГ='ОРГАНИЗАЦИОННОЕ';
function оргЛи(з){ return String(з.дис||'').toUpperCase().indexOf(ОРГ)===0; }
...
function экранСегодня(){
  ...
  if(дела.length){
    ...
    // Организационное идёт первым и отдельной секцией: пропуска, оплата и справки
    // не привязаны к предмету, но пропустить их дороже, чем задание по курсу (14.09).
    var орг=близкие.filter(оргЛи), учебные=близкие.filter(function(з){return !оргЛи(з);});
    if(орг.length){
      эк.appendChild(эл('div','секц','Организационное'));
      орг.forEach(function(з){эк.appendChild(картаЗадания(з,false,true,true));});
    }
    if(учебные.length){
      эк.appendChild(эл('div','секц','Ближайшие задания'));
      учебные.slice(0,ПОТОЛОК_ДЕЛ).forEach(function(з){эк.appendChild(картаЗадания(з,false,true,true));});
    }
    if(близкие.length){
      // обе секции уже нарисованы выше
    }else{
      ...
      эк.appendChild(эл('div','мелко','Ближайшее – '+экр(подписьДаты(дела[0].срок))));
    }
...
function подготовитьДело(з){
  ...
  return {что:з.что, дис:(з.дис||'').split('·')[0].trim(), кто:з.кто, ком:з.ком, комк:з.комк, мат:з.мат,
          срок:с, n:база? d0(база) : 99999, кл:классСрока(с)};
}
/* Кнопки задания. Главная — материал предмета. Рядом, обводкой, — каждая ссылка,
   вшитая в «Комментарий»: в тексте она объясняется словами, а кнопкой её видно сразу,
   не разворачивая описание (просьба владельца 12.09). Подпись кнопки — те же слова,
   что подчёркнуты в ячейке, без хвостовой точки. Нет ссылок — нет и кнопок. */
function ссылкиКомментария(з){
  var из=[], к=з.комк||[];
  for(var i=0;i<к.length;i++) if(к[i].у){
    var п=String(к[i].т||'').replace(/^[\s–—-]+|[\s.,;:–—-]+$/g,'');
    из.push({у:к[i].у, п:п ? п.charAt(0).toUpperCase()+п.slice(1) : 'Ещё ссылка'});
  }
  return из;
}
function кнопкиЗадания(з){
  var доп=ссылкиКомментария(з);
  if(!з.мат && !доп.length) return '';
  var h='<div class="низ">';
  if(з.мат) h+='<a class="кнопка главная" data-сс="'+экр(з.мат)+'">'+(своя(з.мат)?'Материалы предмета':'Открыть материал')+'</a>';
  for(var i=0;i<доп.length;i++) h+='<a class="кнопка" data-сс="'+экр(доп[i].у)+'">'+экр(доп[i].п)+'</a>';
  return h+'</div>';
}
...
/* Ссылки внутри «Комментария» (решение владельца 12.09: вторая ссылка живёт в тексте,
   а не отдельной колонкой). Главный путь — ссылка вшита в слова ячейки (rich text),
   API отдаёт её кусками `комк`: в таблице фраза читается как обычный текст, кликается
   ровно название. Запасной путь — голый адрес в тексте, его линкуем целиком. */
function текстСоСсылками(текст){
  // Вшитые ссылки в описании НЕ подчёркиваем: каждая из них уже вынесена кнопкой
  // в нижний ряд (решение владельца 12.09) — два пути к одному адресу в одной карточке
  // только сбивают. Голый адрес, набранный прямо в тексте, кнопки не получает,
  // поэтому его линкуем здесь.
  return экр(текст).replace(/(^|[\s(])(https?:\/\/[^\s<]+)/g, function(_,до,у){
    return до+'<a href="'+у+'" target="_blank" rel="noopener">'+у+'</a>'; });
}
/* Клик по ссылке не должен разворачивать или схлопывать карточку. */
function неТрогатьКарточку(корень){
  var сс=корень.querySelectorAll('.мелко.текст a');
  for(var i=0;i<сс.length;i++) сс[i].onclick=function(e){ e.stopPropagation(); };
}
...
function картаЗадания(з, прошлое, сПредметом, полная){
  ...
  var есть=!!(т||з.мат||ссылкиКомментария(з).length);
  ...
  c.appendChild(эл('div','задание',
    ...
    (есть?'<div class="детали" hidden>'+(т?'<div class="мелко текст">'+текстСоСсылками(т)+'</div>':'')+
      кнопкиЗадания(з)+'</div>':'')));
  if(есть){
    ...
    навеситьКнопкиЗадания(c, з); неТрогатьКарточку(c);
  }
  return c;
}
function картаЗаданияПолная(з){
  ...
  c.appendChild(эл('div','дело',
    ...
    (т?'<div class="мелко текст'+(длинно?' свёрнут':'')+'">'+текстСоСсылками(т)+'</div>':'')+
    кнопкиЗадания(з)));
...
function списокДел(){
  ...
  // организационное — всегда первым блоком, чтобы не терялось среди предметов (14.09)
  порядок.sort(function(a,b){ return (String(b).toUpperCase().indexOf(ОРГ)===0) - (String(a).toUpperCase().indexOf(ОРГ)===0); });
...
function экранКниг(){
  ...
  /* Два ряда, оба видны всегда (решение владельца 12.09): сверху ★ и предметы, снизу
     разделы. Предмет не выбран — в нижнем ряду разделы всех предметов, как было до 12.09;
     выбран — только его разделы. Раздел, который повторяет имя предмета, в ряд не ставим:
     он дублировал бы чип сверху. */
  поПорядку(Д.книги,'д').forEach(function(д){
    var к=ключДис(д);
    var ч=чип(короткийПредмет(д), фДис===к, function(){ фДис=(фДис===к?'':к); фРаздел=''; отрисовать(); });
    ряд.appendChild(ч);
  });
  эк.appendChild(ряд); показатьВыбранныйЧип(ряд);
  var свои=фДис ? Д.книги.filter(function(к){return ключДис(к.д)===фДис;}) : Д.книги;
  var разделы=поПорядку(свои,'б').filter(function(б){
    return !свои.some(function(к){return к.б===б && имяКапса(к.д||'')===имяКапса(б);});
  });
  if(разделы.length){
...
/** Подпись предмета в чипе: длинные названия режем, «психологию» не трогаем —
    без неё «Общая» и «Социальная» ничего не значат (12.09). */
function короткийПредмет(с){
  var и=имяКапса(с);
  return и.replace(/^Информационные технологии.*/i,'ИТ')
          .replace(/^Психофизиология.*/i,'Психофизиология')
          .replace(/^Правовое регулирование.*/i,'Право')
          .replace(/^Идеология.*/i,'Идеология')
          .replace(/^Охрана труда.*/i,'Охрана труда');
}
function имяКапса(с){
  с=String(с||'');
