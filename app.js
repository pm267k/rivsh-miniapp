...
function экранПредмета(ключ){
  ...
  кр.onclick=function(){ вибро(); открытыйПредмет=null; перейти('пары'); };
  эк.appendChild(кр);

  // «Другие материалы» (папка с раздатками) — последней в списке, сразу за ней книги
  var мат=у.мат.filter(function(м){return м.что!=='Другие материалы';})
               .concat(у.мат.filter(function(м){return м.что==='Другие материалы';}));
  if(мат.length||у.книги){
    эк.appendChild(эл('div','секц','Материалы'));
    мат.forEach(function(м){ эк.appendChild(рядСо(м.что, м.тип||'', function(){ открыть(м.сс); }, !м.сс)); });
  ...
  if(ещё>0){
    ...
    кз.onclick=function(){ вибро(); фСрок=''; фПредметЗ=дела[0].дис; открытыйПредмет=null; перейти('дела'); };
    эк.appendChild(кз);
  }
}
