...
function текстСоСсылками(текст, куски){
  if(куски && куски.length){
    var out='';
    for(var i=0;i<куски.length;i++){
      var к=куски[i];
      out += к.у ? '<a href="'+экр(к.у)+'" target="_blank" rel="noopener">'+экр(к.т)+'</a>' : экр(к.т);
    }
    return out;
  }
...
function картаЗадания(з, прошлое, сПредметом, полная){
  ...
  c.appendChild(эл('div','задание',
    ...
    (есть?'<div class="детали" hidden>'+(т?'<div class="мелко текст">'+текстСоСсылками(т, з.комк)+'</div>':'')+
      кнопкиЗадания(з)+'</div>':'')));
  if(есть){
...
function картаЗаданияПолная(з){
  ...
  c.appendChild(эл('div','дело',
    ...
    (т?'<div class="мелко текст'+(длинно?' свёрнут':'')+'">'+текстСоСсылками(т, з.комк)+'</div>':'')+
    кнопкиЗадания(з)));
