...
function картаДела(з){
  ...
  // предмет и преподаватель — одной меткой: дата уже стоит чипом справа, дублировать её незачем
  var метка=[з.дис||'без дисциплины', сокрФамилию(з.кто)].filter(Boolean).join(' · ');
...
/** «Бараева Е.И.» → «Бараева»: в метке важна фамилия, инициалы её только утяжеляют. */
function сокрФамилию(кто){ return String(кто||'').trim().split(/[\s·]+/)[0]; }
function экранДел(){
