...
function отрисовать(){
  ...
  эк.appendChild(эл('div','свежесть','данные из таблицы группы'+(Д.обновлено?' · '+экр(Д.обновлено):'')));
}

/* ---------- Сегодня ---------- */
function экранСегодня(){
  var п=ДНИ[сегодня.getDay()]; п=п.charAt(0).toUpperCase()+п.slice(1);
  var неделя=(Д.расписание.filter(function(x){return дденьМесяца(x.д)&&ддмм(дденьМесяца(x.д))===ддмм(сегодня);})[0]||{}).н||'';
  эк.appendChild(эл('div','шапка','<h2>'+экр(п)+', '+сегодня.getDate()+' '+МЕС_РОД[сегодня.getMonth()]+'</h2>'+
    (неделя?'<span>'+экр(неделя.split('·')[0].trim())+'</span>':'')));
  ...
  if(сег.length){
    ...
    сег.forEach(function(x){эк.appendChild(картаПары(x,true));});
  }else{
    ...
    if(буд.length){
      ...
      Д.расписание.filter(function(x){return x.д===буд[0].д;}).forEach(function(x){эк.appendChild(картаПары(x,false));});
    }
  }
...
function картаПары(п,акцент){
  var c=эл('div','карта'+(акцент?' акцент':''));
  c.appendChild(эл('div','пара','<div class="время">'+экр(п.вр).replace('–','<br>')+'</div>'+
    '<div><div class="дис">'+экр(п.дис)+(п.вид&&п.вид!=='лекция'?'<span class="бейдж">'+экр(п.вид)+'</span>':'')+
    '</div><div class="мелко">'+экр(п.преп||'—')+'</div></div>'));
  return c;
}
...
function экранПары(){
  эк.appendChild(эл('div','шапка','<h2>Расписание</h2><span>'+Д.расписание.length+' пар</span>'));
  ...
  порядок.forEach(function(ключ){
    ...
    поДням[ключ].forEach(function(п){
      var c=картаПары(п,сег); if(прошло) c.className+=' тихо'; эк.appendChild(c);
    });
  });
...
function экранДел(){
  ...
  var группы=[
    ['Ближайшее', все.filter(function(з){return з.n>=0&&з.n<=14;})],
  ...
  группы.forEach(function(г){
    ...
    г[1].forEach(function(з){var c=картаДела(з); if(г[0]==='Прошло')c.className+=' тихо'; эк.appendChild(c);});
  });
}
...
function экранКниг(){
  эк.appendChild(эл('div','шапка','<h2>Книги</h2><span>'+Д.книги.length+'</span>'));
...
function экранПредметов(){
  ...
  эк.appendChild(эл('div','шапка','<h2>Предметы</h2><span>'+идут.length+' в этой сессии</span>'));
...
function плиткаПредмета(у){
  ...
  b.onclick=function(){ открытыйПредмет=у.ключ; вибро(); отрисовать(); };
  return b;
}
function экранПредмета(ключ){
  ...
  var н=эл('button','назад','← Все предметы');
  н.onclick=function(){ открытыйПредмет=null; вибро(); отрисовать(); };
  эк.appendChild(н);
...
/* ---------- Контакты ---------- */
function экранЛюдей(){
  эк.appendChild(эл('div','шапка','<h2>Контакты</h2><span>'+Д.контакты.length+'</span>'));
  var секции={};
...
/** Экран для тех, кто не в группе. Человек должен понять, куда он попал и что делать. */
function отказДоступа(){
  ошибка(
    '<div style="text-align:left;max-width:34ch;margin:0 auto">' +
    '<p style="margin:0 0 12px">Это внутреннее приложение учебной группы <b>ПМ 2.67к</b> — ' +
    'веб-психология, РИВШ, набор 2026 года.</p>' +
    '<p style="margin:0 0 12px">Здесь расписание, задания и материалы группы, поэтому оно открыто только её участникам.</p>' +
    '<p style="margin:0;color:var(--muted)">Если ты из группы, а приложение не пускает — напиши ' +
    '<a href="https://t.me/constantinserdiuk">@constantinserdiuk</a>.</p>' +
    '</div>', 'Доступ закрыт');
}
